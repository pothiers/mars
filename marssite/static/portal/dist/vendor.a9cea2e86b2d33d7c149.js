  ...
  mixin: {
    ...
    methods: {
      stripData: ()=> {
        ...
        newFormData = this.default.search !== void 0 ? JSON.parse(JSON.stringify(this.default.search)) : JSON.parse(localStorage.getItem("searchData"));
        for (key in newFormData) {
          if (_.isEqual(newFormData[key], this.default.config.formData[key])) {
            delete newFormData[key];
          } else {
            if (this.default.config.rangeInputs.indexOf(key) >= 0) {
      ...
      submitForm: (event, paging, cb)=> {
        ...
        if (!paging) {
          this.loading = true;
          this.url = this.default.config.apiUrl;
        ...
        msgs = this.default.config.loadingMessages;
        message = Math.floor(Math.random() * msgs.length);
        this.loadingMessage = msgs[message];
        self = this;
        url = this.default.config.apiUrl + ("?page=" + page);
